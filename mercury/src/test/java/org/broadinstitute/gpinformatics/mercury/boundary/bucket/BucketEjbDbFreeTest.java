...
public class BucketEjbDbFreeTest {
    ...
    // MercurySample name to barcoded tube.
    private Multimap<String, LabVessel> sampleNameToVesselMap = HashMultimap.create();
    // PDO SampleName (barcode) to barcoded tube.
    private Map<String, LabVessel> barcodeToVesselMap = new HashMap<>();
    ...
    private Object[] mocks = new Object[]{bucketDao, bucketEntryDao, labVesselDao, bspSampleDataFetcher,
    ...
    private void setupMercurySamples(ProductOrder pdo, Collection<ProductOrderSample> expectedSamples,
        ...
        sampleNameToVesselMap.clear();
        barcodeToVesselMap.clear();
        ...
        for (int rackPosition = 1; rackPosition <= pdo.getSamples().size(); ++rackPosition) {
            ...
            case 2:
                // Received root sample but non-genomic material.
            ...
            case 4:
            case 5:
                // Position 4 & 5 non-root samples are known to BSP but not to Mercury.
                hasMercurySample = false;
                bspData.put(BSPSampleSearchColumn.MATERIAL_TYPE, MaterialType.DNA_DNA_GENOMIC.getDisplayName());
            ...
            default:
                Assert.fail("No setup for position " + rackPosition);
            }
            ...
            if (hasMercurySample) {
                barcodeToVesselMap.put(labVessel.getLabel(), labVessel);
                sampleNameToVesselMap.put(mercurySample.getSampleKey(), labVessel);
    ...
    private void setupCoreMocks(Workflow workflow, boolean createVessels) {
        ...
        // Sample at position 1 has not been received.
        // Samples at positions 2 and 3 are known to Mercury.
        // Samples at positions 4 and 5 are BSP aliquots and the test will create a tube and MercurySample for them.
        ...
        for (int rackPosition = 1; rackPosition <= SAMPLE_SIZE; ++rackPosition) {
            ...
            if (rackPosition == 2 || rackPosition == 3) {
        ...
        expect(labVesselDao.findMapBySampleKeys(EasyMock.<Collection<ProductOrderSample>>anyObject()))
                .andReturn(sampleNameToVesselMap);

        expect(labVesselDao.findByBarcodes(EasyMock.<List<String>>anyObject())).andReturn(barcodeToVesselMap);
