...
public class ReagentKitTransferTest  {
    ...
    public void testDenatureToReagentKit() {
        ...
        for (int i = 1; i <= 8; i++) {
            denatureTubeBarcodes.add(String.format("denatureTube0%s-%s",i,time));
        }
        String miSeqReagentKitBarcode = "reagentKit" + time;

        List<BettaLimsMessageTestFactory.CherryPick> cherryPicks = Arrays.asList(
                new BettaLimsMessageTestFactory.CherryPick(denatureRackBarcode,
                        VesselPosition.A01.name(), miSeqReagentKitBarcode, "D04"));
        ...
        BettaLimsMessageTestFactory factory = new BettaLimsMessageTestFactory(true);
