...
<%--@elvariable id="labSearchResultList" type="org.broadinstitute.gpinformatics.infrastructure.columns.ConfigurableList.ResultList"--%>
    <div class="form-inline">
        <c:if test="${fn:length(actionBean.searchValuesNotFound) > 0}">
            <h3 style="margin-top: 50px;text-align:center;">${fn:length(actionBean.searchValuesNotFound)} searched values listed below were not found in the queue at all.</h3>
        ...
        <div id="successfulSearchStatus" style="margin-top: 50px;text-align:center;">
<%--            If there are no search values 'not found' and result rows is not empty, display a success message.--%>
            <c:if test="${empty actionBean.searchValuesNotFound && empty actionBean.labSearchResultList.resultRows}">
