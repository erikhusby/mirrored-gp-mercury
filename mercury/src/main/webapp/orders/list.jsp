...
<div style="display:none;" id="confirmDialog">
    <p>Are you sure you want to <span id="confirmDialogMessage"></span> the <span id="dialogNumProductOrders"></span>
        Product Order(s)?</p>
</div>
    ...
    <div class="search-horizontal">
            ...
            <div class="controls">
                <c:forEach items="<%=ProductOrder.OrderStatus.values()%>" var="orderStatus">
                    <div style="margin-top: 5px; margin-right: 15px; float: left; width: auto;">
                        <stripes:checkbox onchange="statusChange()" class="search-checkbox selectedStatuses"
                                          name="selectedStatuses" value="${orderStatus}" id="${orderStatus}-id"/>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <div id="draftMessage" class="help-text"
                     style="margin-left: 10px;margin-top: -10px; margin-bottom: 5px; display: none;">
        ...
        <div class="control-group">
            ...
            <div class="controls actionButtons">
                <stripes:submit name="list" value="Search" style="margin-right: 10px;margin-top:10px;"
                                class="btn btn-mini"/>
            </div>
        </div>
    </div>
...
<stripes:form beanclass="${actionBean.class.name}" id="createForm" class="form-horizontal">
    ...
    <div class="actionButtons">
        ...
        <security:authorizeBlock roles="<%= roles(Developer) %>">
            <stripes:button name="abandonOrders" value="Abandon Orders" class="btn"
                            onclick="showConfirm('abandonOrders', 'abandon')" style="margin-right:30px;"/>
            ...
            <tr>
                <td>
                    <c:if test="${!order.draft}">
                        <stripes:checkbox class="shiftCheckbox" name="selectedProductOrderBusinessKeys"
                                          value="${order.businessKeyList}"/>
                    </c:if>
                </td>
                <td>
                    <stripes:link beanclass="${actionBean.class.name}" event="view">
                        <stripes:param name="productOrder" value="${order.businessKeyList}"/>
                        ${order.title}
                    </stripes:link>
                </td>
                <td>
                ...
                <td align="center">
                    <div class="barFull"
                         title="${actionBean.progressFetcher.getPercentInProgress(order.businessKeyList)}% In Progress">
                                    <span class="barAbandon"
                                          title="${actionBean.progressFetcher.getPercentAbandoned(order.businessKeyList)}% Abandoned"
                                          style="width: ${actionBean.progressFetcher.getPercentAbandoned(order.businessKeyList)}%"> </span>
                                    <span class="barComplete"
                                          title="${actionBean.progressFetcher.getPercentCompleted(order.businessKeyList)}% Completed"
                                          style="width: ${actionBean.progressFetcher.getPercentCompleted(order.businessKeyList)}%"> </span>
                    </div>
                </td>
                <td>${actionBean.progressFetcher.getNumberOfSamples(order.businessKeyList)}</td>
                <td>
